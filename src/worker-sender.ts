    ...
    while (1) {
      ...
      switch (payload.type) {
        ...
        case 4:
          ...
          break;
        case 5:
          payload = <components["schemas"]["PushNotificationLightningInvoiceUserPaid"]>payload;
          process.env.VERBOSE && console.warn("pushing to token", payload.token, payload.os);
          await GroundControlToMajorTom.pushLightningInvoiceUserPaid(connection, GroundControlToMajorTom.getGoogleServerKey(), GroundControlToMajorTom.getApnsJwtToken(), payload);
          await sendQueueRepository.remove(record);
          break;
        default:
